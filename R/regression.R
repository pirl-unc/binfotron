...
#' 
#' @export
regression <- function (input_dt, indep_list, base_file_name = "regression_output", 
                        clear_readme = TRUE, combined_group_name = "All", dep_vars = NULL, 
                        dep_var_families = NULL, event_clm = "OS_e", fdr_by_columns = NULL, 
                        fdr_by_columns_for_model_comp = NULL, fdr_method = "BH", 
                        inclusion_list = list(), model_comparison_list = NULL, model_function = function(dep_var = "", 
                                                                                                         indep_vars = "NULL") {
                          paste0("glm(", dep_var, " ~ ", paste0(indep_vars, collapse = " + "), 
                                 ", data = model_dt)")
                        }, my_grouping = NULL, output_dir = ".", sample_clm = "Run_ID", 
                        save_models = FALSE, time_clm = "OS_d", write_files = TRUE, 
                        include_dep_var_in_prediction_name = FALSE) 
{
  library(checkmate)
  if (!dir.exists(dirname(output_dir))) 
    stop("The parent directory of output_dir does not exist.")
  dir.create(output_dir, showWarnings = F)
  stats_path = file.path(output_dir, paste0(base_file_name, 
                                            "_stats.tsv"))
  if (file.exists(stats_path)) 
    file.remove(stats_path)
  model_comparison_path = file.path(output_dir, paste0(base_file_name, 
                                                       "_models.tsv"))
  if (file.exists(model_comparison_path)) 
    file.remove(model_comparison_path)
  model_prediction_path = file.path(output_dir, paste0(base_file_name, 
                                                       "_predictions.tsv"))
  if (file.exists(model_prediction_path)) 
    file.remove(model_prediction_path)
  readme_path = file.path(output_dir, paste0(base_file_name, 
                                             "_readme.txt"))
  if (clear_readme) 
  ...
  if (grepl("coxph", model_function())) {
    ...
    library(survival)
    input_dt = input_dt[complete.cases(input_dt[, .SD, .SDcol = c(event_clm, 
                                                                  time_clm)]), ]
    is_coxph = TRUE
    dep_vars = "Survival"
    ...
    for (dep_var in dep_vars) {
      if (column_classes[dep_var] %in% c("factor", "character")) 
        stop(paste0("Dependent variable, ", dep_var, 
                    ", cannot be a factor or character class."))
      if (length(unique(input_dt[[dep_var]])) < 2) 
        stop(paste0("Dependent variable, ", dep_var, 
                    ", only has one value in it's column."))
    }
    if (length(dep_var_families) != 1 & (length(dep_var_families) != 
                                         length(dep_vars))) 
  ...
  try_error_warning = function(try_string_eval, my_env) {
    ...
    return(list(return_value = my_value, warn = a_warn, warn_msg = warn_msg, 
  ...
  get_glm_stats = function(my_model) {
    ...
    output_dt = rbindlist(lapply(2:nrow(coef_mtrx), function(coef_index) {
      ...
      pValue_column = summary_columns[grepl("Pr(>|t|)", 
                                            summary_columns, fixed = T) | grepl("Pr(>|z|)", 
                                                                                summary_columns, fixed = T)]
      output_list["pValue"] = coef_mtrx[coef_index, pValue_column]
      output_list["Coef"] = coef_mtrx[coef_index, "Estimate"] %>% 
        as.numeric
          ...
          for (x_index in 1:length(my_model$xlevels)) {
            ...
            for (my_level in my_levels[2:length(my_levels)]) {
              ...
              if (try_me == coefficient_name) {
                ...
                output_list["Coefficient_Name"] = paste0(my_var, 
                                                         ": ", my_level, " vs ", my_levels[1])
                break
              }
            }
            if (found_it) 
      ...
      if (coef_class %in% c("numeric", "integer")) {
        ...
        TF_counts = TF_counts[TF_counts %>% order %>% 
                                rev]
        my_n = paste0(TF_counts, collapse = ":")
      }
      else if (coef_class == "factor") {
        my_n = paste0(summary(this_coef_data)[coef_index], 
                      ":", summary(this_coef_data)[[1]])
      }
      else if (coef_class == "ordered") {
        ordinal_level = substr(coefficient_name, nchar(coefficient_name), 
                               nchar(coefficient_name))
        if (ordinal_level == "L") {
          ordinal_level = "1"
        }
        ...
        my_n = paste0(summary(this_coef_data)[ordinal_level + 
                                                1], ":", summary(this_coef_data)[[ordinal_level]])
      }
      else {
        stop("coef_class != numeric or integer or logical. need to step through get_stats for this class")
      }
      output_list["N"] = my_n
      if (coef_class %in% c("logical", "numeric", "integer", 
                            "factor")) {
        confint_list = tryCatch({
          suppressMessages(list(df = confint(my_model, 
                                             level = 0.95)))
        }, warning = function(w) {
          suppressWarnings(list(df = confint(my_model, 
                                             level = 0.95), warning = paste(w)))
        }, error = function(e) {
          error_text = gsub("\n", "", paste(e))
        })
        if ("df" %in% names(confint_list)) {
          output_list["Lower_CI"] = confint_list$df[coefficient_name, 
                                                    "2.5 %"]
          output_list["Upper_CI"] = confint_list$df[coefficient_name, 
                                                    "97.5 %"]
        }
        if ("warning" %in% names(confint_list)) 
          output_list["CI_Warn"] = paste0(confint_list$warning)
      }
      else {
        warning_msg = paste0("Confidence intervals have not been tested for the variable type: ", 
    ...
    if (my_model$nevent > 0) {
      ...
      output_dt = rbindlist(lapply(1:nrow(coef_mtrx), function(coef_index) {
        ...
        pValue_column = summary_columns[grepl("Pr(>|t|)", 
                                              summary_columns, fixed = T) | grepl("Pr(>|z|)", 
                                                                                  summary_columns, fixed = T)]
        output_list["pValue"] = coef_mtrx[coef_index, 
                                          pValue_column]
        output_list["Coef"] = coef_mtrx[coef_index, "coef"] %>% 
          as.numeric
        output_list["Events"] = paste0(my_model$nevent)
        output_list["Non_Events"] = paste0(my_model$n - 
                                             my_model$nevent)
        output_list["Hazard_Ratio"] = summary(my_model)$coefficients[coef_index, 
                                                                     "exp(coef)"] %>% as.numeric
        output_list["HR_Lower_CI"] = summary(my_model, 
                                             conf.int = 0.95)$conf.int[coef_index, 3] %>% 
          as.numeric
        output_list["HR_Upper_CI"] = summary(my_model, 
                                             conf.int = 0.95)$conf.int[coef_index, 4] %>% 
          as.numeric
        return(output_list)
      }), use.names = T, fill = T)
    }
    else {
      output_dt = data.table(pValue = NA, Coef = NA, N = paste0("0:", 
                                                                my_model$n), Hazard_Ratio = NA, HR_Lower_CI = NA, 
                             HR_Upper_CI = NA)
    }
    return(output_dt)
  }
  if (is_coxph) {
    required_col = unique(c(my_grouping, names(inclusion_list), 
                            time_clm, event_clm, unlist(indep_list), unlist(model_comparison_list)))
  } else {
    required_col = unique(c(my_grouping, names(inclusion_list), 
                            dep_vars, unlist(indep_list), unlist(model_comparison_list)))
  }
  ...
  for (group_index in 1:length(my_groups)) {
    ...
    if (my_group != combined_group_name) {
      ...
      if (my_grouping %in% unique(unlist(model_comparison_list))) {
        if (show_group_in_model_warning) {
          ...
          show_group_in_model_warning = FALSE
        }
        doesnt_include_grouping = sapply(model_comparison_list, 
                                         function(x) {
                                           my_grouping %ni% x
                                         })
    ...
    for (dep_index in 1:length(dep_vars)) {
      ...
      if (is_coxph) {
        dep_var_dat = group_dt[complete.cases(group_dt[, 
                                                       .SD, .SDcols = c(time_clm, event_clm)]), ]
      }
      else {
        ...
        if (nrow(dep_var_dat) < 2) {
          a(paste0("    - Dependent variable, '", dep_var, 
                   "', has fewer than two complete cases. Skipping"))
          next
        }
        else if (num_levels < 2) {
          a(paste0("    - Dependent variable, '", dep_var, 
                   "', has fewer than two levels. Skipping"))
          next
        }
      }
      model_function_w_family = function(raw_string) {
        if (!is_coxph) {
          return(gsub(")$", paste0(", family = ", dep_var_families[dep_index], 
                                   " )"), raw_string))
        }
        else {
          return(raw_string)
      ...
      for (indep_index in 1:length(indep_list)) {
        model_dt = dep_var_dat[complete.cases(dep_var_dat[, 
                                                          unique(c(indep_list[[indep_index]], unlist(this_groups_model_comparison_list))), 
                                                          with = FALSE]), ]
        ...
        if (!is_coxph && length(unique(model_dt[[dep_var]])) < 
            2) {
          a(paste0("      - Dependent variable, '", dep_var, 
                   "', has fewer than two levels for this independent variable. Skipping"))
          next
        }
        if (nrow(model_dt) < 2) {
          a(paste0("      - Independent variable, '", 
                   indep_var, "', has fewer than two complete cases (including model comparisons). Skipping"))
          next
        }
        else {
          ...
          if (!enough_indep_levels) {
            a(paste0("      - Independent variable, '", 
                     my_indep_name, "', has fewer than two levels for this dependent variable. Skipping"))
            next
          }
        }
        ...
        if (try_model$error) {
          my_dt = data.table(Independent = my_indep_name, 
                             Dependent = dep_var, Group = my_group, Independent_Var = paste0(indep_var, 
                                                                                             collapse = ","), String = model_str, Error = try_model$error, 
                             Warning = try_model$warn, Error_Msg = try_model$error_msg, 
          ...
          if (include_dep_var_in_prediction_name) {
            model_name = paste0(my_group, "__", dep_var, 
                                "_vs_", my_indep_name)
          }
          else {
          ...
          if (save_models) 
            saveRDS(model_name, file = file.path(output_dir, 
                                                 paste0(base_file_name, "_", model_name, 
                                                        "_model.rds")))
          my_dt = data.table(Independent = my_indep_name, 
                             Dependent = dep_var, Group = my_group, Independent_Var = paste0(indep_var, 
                                                                                             collapse = ","), get_stats(my_model = my_model), 
                             String = model_str, Error = try_model$error, 
                             Warning = try_model$warn, Error_Msg = try_model$error_msg, 
                             Warning_Msg = try_model$warn_msg)
          prediction_clm = paste0(model_name, "_Prediction")
          model_dt[[prediction_clm]] = predict(my_model, 
                                               model_dt)
          prediction_model_str = model_function_w_family(model_function(dep_var, 
                                                                        prediction_clm))
          try_model = try_error_warning(prediction_model_str, 
                                        my_env = environment())
          my_model = try_model$return_value
          if (nrow(predictions_dt) == 0) {
            predictions_dt = model_dt[, c(sample_clm, 
                                          prediction_clm), with = F]
          }
          else {
            predictions_dt = merge(predictions_dt, model_dt[, 
                                                            c(sample_clm, prediction_clm), with = F], 
                                   by = sample_clm, all = T)
          }
          if (length(indep_var) > 1) {
            if (try_model$error) {
              combined_dt = data.table(Independent = my_indep_name, 
                                       Dependent = dep_var, Group = my_group, 
                                       Independent_Var = paste0(indep_var, collapse = ","), 
                                       Coefficient_Name = prediction_clm, String = prediction_model_str, 
                                       Error = try_model$error, Warning = try_model$warn, 
                                       Error_Msg = try_model$error_msg, Warning_Msg = try_model$warn_msg, 
                                       Complete_Model = TRUE)
              if (is_coxph) {
                combined_dt$Events = sum(model_dt[[event_clm]] == 
                                           1)
                combined_dt$Non_Events = sum(model_dt[[event_clm]] == 
                                               0)
              }
              else {
              ...
              combined_dt = data.table(Independent = my_indep_name, 
                                       Dependent = dep_var, Group = my_group, 
                                       Independent_Var = paste0(indep_var, collapse = ","), 
                                       get_stats(my_model = my_model), String = prediction_model_str, 
                                       Error = try_model$error, Warning = try_model$warn, 
                                       Error_Msg = try_model$error_msg, Warning_Msg = try_model$warn_msg, 
                                       Complete_Model = TRUE)
            }
            my_dt$Complete_Model = FALSE
            my_dt = rbindlist(list(my_dt, combined_dt), 
                              use.names = T, fill = T)
          }
          else {
            my_dt$Complete_Model = TRUE
          }
          if (!is.null(this_groups_model_comparison_list) && 
              length(this_groups_model_comparison_list) > 
              0) {
            this_comp_dt = rbindlist(lapply(1:length(this_groups_model_comparison_list), 
                                            function(comp_index) {
                                              my_comp_var = this_groups_model_comparison_list[[comp_index]]
                                              my_comp_name = names(this_groups_model_comparison_list)[comp_index]
                                              a(paste0("      - Model comparison: ", 
                                                       my_comp_name))
                                              full_model_str = model_function_w_family(model_function(dep_var, 
                                                                                                      c(my_comp_var, indep_var)))
                                              try_full = try_error_warning(full_model_str, 
                                                                           my_env = environment())
                                              full_model = try_full$return_value
                                              reduced_model_str = model_function_w_family(model_function(dep_var, 
                                                                                                         c(my_comp_var)))
                                              try_reduced = try_error_warning(reduced_model_str, 
                                                                              my_env = environment())
                                              reduced_model = try_reduced$return_value
                                              if (try_full$error && try_reduced$error) {
                                                return(list(Independent = my_indep_name, 
                                                            Dependent = dep_var, Group = my_group, 
                                                            Model_Comparison = my_comp_name, 
                                                            Independent_Var = paste0(indep_var, 
                                                                                     collapse = ","), Comparison_Model_Var = paste0(my_comp_var, 
                                                                                                                                    collapse = ","), N = nrow(model_dt), 
                                                            Full_Model = full_model_str, Reduced_Model = reduced_model_str, 
                                                            Full_Error = try_full$error, Full_Warning = try_full$warn, 
                                                ...
                                                a1 <- anova(full_model, reduced_model, 
                                                            test = "LRT")
                                                a1_columns = names(a1)
                                                a1_pValue_column = a1_columns[grepl("Pr(>Chi)", 
                                                                                    a1_columns, fixed = T) | grepl("P(>Chi)", 
                                                                                                                   a1_columns, fixed = T) | grepl("P(>|Chi|)", 
                                                                                                                                                  a1_columns, fixed = T)]
                                                pval <- a1[2, a1_pValue_column]
                                                logLik_full = logLik(full_model) %>% 
                                                  as.numeric
                                                logLik_red = logLik(reduced_model) %>% 
                                                  as.numeric
                                                return(list(Independent = my_indep_name, 
                                                            Dependent = dep_var, Group = my_group, 
                                                            Model_Comparison = my_comp_name, 
                                                            Independent_Var = paste0(indep_var, 
                                                                                     collapse = ","), Comparison_Model_Var = paste0(my_comp_var, 
                                                                                                                                    collapse = ","), Full_LogLik = logLik_full, 
                                                            Reduced_LogLik = logLik_red, Delta_LogLik = logLik_full - 
                                                              logLik_red, pValue = pval, N = nrow(model_dt), 
                                                            Full_Model = full_model_str, Reduced_Model = reduced_model_str, 
                                                            Full_Error = try_full$error, Full_Warning = try_full$warn, 
                                                            Reduced_Error = try_reduced$error, 
                                                            Reduced_Warning = try_reduced$warn, 
  ...
  if (fdr_method %in% p.adjust.methods[p.adjust.methods != 
                                       "none"]) {
    a("FDR correcting regression pValues.")
    pvalue_dt = calc_fdr(my_dt = pvalue_dt, fdr_by_columns = fdr_by_columns, 
                         fdr_method = fdr_method, fdr_on_columns = "pValue", 
                         readme_path = readme_path)
  }
  pvalue_dt = decode_clms(pvalue_dt, skip_clms = "String")
  if (write_files) 
    fwrite(pvalue_dt, stats_path, quote = FALSE, sep = "\t", 
  ...
  if (nrow(comp_dt) > 0) {
    ...
    if (fdr_method %in% p.adjust.methods[p.adjust.methods != 
      ...
      comp_dt = calc_fdr(my_dt = comp_dt, fdr_by_columns = fdr_by_columns_for_model_comp, 
                         fdr_method = fdr_method, fdr_on_columns = "pValue", 
                         readme_path = readme_path)
    }
    if (write_files) {
    ...
    if (write_files) {
      fwrite(comp_dt, model_comparison_path, quote = FALSE, 
             sep = "\t", col.names = TRUE, na = "NA")
      fwrite(predictions_dt, model_prediction_path, quote = FALSE, 
             sep = "\t", col.names = TRUE, na = "NA")
    }
    else {
      file.remove(readme_path)
    ...
    return(list(stats = pvalue_dt, readme = readme_content, 
                models = output_models, predictions = predictions_dt))
  }
}
