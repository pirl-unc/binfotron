...
view_clustered_myHeatmap = function(# passed from analysis app
  ...
  # 
  # if(base_for_log_transform %>% is_not_null){
  #   input_df = log((input_df + 1), base = base_for_log_transform)
  #   a(paste0("Log transformed data+1 using base ", base_for_log_transform, "."))
  # }
  # 
  # 
  # if(order_by_fold_change & fold_change %>% is_not_null){
  #   a("Ordering data based on fold change.")
  #   new_order = rev(order(fold_change))
  #   fold_change = fold_change[new_order]
  #   input_df = input_df[ , names(fold_change)]
  #   pValue = pValue[new_order]
  #   FDR_pValue = FDR_pValue[new_order]
  # }
  # 
  # 
  # remove_clm_indices = NULL
  # max_significance = ""
  # # limit features displayed based on pvalue threshholds
  # if(pValue_threshhold %>% is_not_null){
  #   if(pValue_threshhold %>% is_not_na){
  #     a(paste0("Removing features with pValue less than ", pValue_threshhold, "."))
  #     over_pValue_indices = which(pValue > pValue_threshhold)
  #     over_pValue_indices = c(over_pValue_indices, which(pValue %>% is.na))
  #     if(length(over_pValue_indices) > 0){
  #       remove_clm_indices = c(remove_clm_indices, over_pValue_indices)
  #     }
  #   }
  # }
  #   
  # if (FDR_threshhold %>% is_not_null ){
  #   if (FDR_threshhold %>% is_not_na){
  #     max_significance = pvalue_stars(min(FDR_pValue, na.rm = T))
  #     
  #     a(paste0("Removing features with FDR adjusted pValue less than ", FDR_threshhold, "."))
  #     over_FDR_indices = which(FDR_pValue > FDR_threshhold)
  #     over_FDR_indices = c(over_FDR_indices, which(FDR_pValue %>% is.na))
  #     if(length(over_FDR_indices) > 0){
  #       remove_clm_indices = c(remove_clm_indices, over_FDR_indices)
  #       remove_clm_indices = unique(remove_clm_indices)
  #     }
  #   }
  # }
  # 
  # if(length(remove_clm_indices) > 0){
  #   input_df = input_df[,-remove_clm_indices, drop = F]
  #   fold_change = fold_change[-remove_clm_indices]
  #   pValue = pValue[-remove_clm_indices]
  #   FDR_pValue = FDR_pValue[-remove_clm_indices]
  # }
  # 
  # a(paste0("Final graph has ", ncol(input_df), " features."))
  # 
  # output_df = data.frame(sample_names, groups, input_df)
  # names(output_df) = c("Sample", "Category", names(input_df))
  # output_df$Sample =output_df$Sample %>% as.character
  # 
  # if(ncol(input_df) == 0){
  #   stop("No features are in the data frame. Perhaps make the FDR or pValue threshhold less stringent.")
  # }
  # 
  # gene_names = names(input_df) # 'apply' can mess up column names if they have a '|' so we need to save them
  # 
  # fold_change_df = data.frame(Gene = gene_names, Fold_Change = fold_change, pValue = pValue, FDR_pValue = FDR_pValue)
  # 
  # write.table(fold_change_df, file.path(output_dir, paste0(base_file_name, "_fold_change.txt")), sep = "\t",
  #             quote = FALSE, row.names = FALSE, col.names = TRUE)
  # 
  # #gene_names = pretty_gene_name(names(input_df)) # 'apply' can mess up column names if they have a '|' so we need to save them
  # #names(input_df) = gene_names
  # 
  # # a("Droping duplicates when gene names show up more than once, which can happen if a transcript has one mgi or hgnc but multiple entrez numbers." %>% wrap_sentence)
  # 
  # # scale across genes
  # a("Scale and center row data for grid showing individual samples.")
  # a("")
  # scaled_df = data.frame(apply(input_df, 2, scale)) # screws up names
  # names(scaled_df) = gene_names
  # 
  # # get min and max for setting the scale bar
  # min_sig = min(scaled_df[is_not_na(scaled_df)])
  # max_sig = max(scaled_df[is_not_na(scaled_df)])
  # extreme_sig = max(abs(max_sig), abs(min_sig))
  # 
  # if(append_classifier_to_sample_name){ # may get rid of this.  not really needed since adding facet labels
  #   scaled_df$Sample_ID = paste0(sample_names, ' ', groups)
  #   
  # } else {
  #   scaled_df$Sample_ID = sample_names
  # }
  # scaled_df$groups = groups
  # if(average_scaled_values){
  #   a("Average scaled sample values to produce average value of samples belonging to each category." %>% wrap_sentence)
  #   a("")
  #   average_df = scaled_df[,1:(ncol(scaled_df)-2)]
  #   average_colors = zscore_colors
  #   ave_legend_name = "Z-Score"
  # } else {
  #   a("Average unscaled sample values to produce average value of samples belonging to each category." %>% wrap_sentence)
  #   a("")
  #   average_df = input_df
  #   ave_legend_name = "Ave"
  # }
  # 
  # 
  # scaled_df = gather(scaled_df, 'Gene', 'ZScore', 1:(ncol(scaled_df)-2))
  # 
  # # scaled_df$Sample_ID = factor(scaled_df$Sample_ID, levels = paste0(sample_names, ' ', groups))
  # 
  # scaled_df$Gene = factor(scaled_df$Gene, levels = rev(gene_names)) # backwards because the plot will start from the bottom up
  # if(show_sample_names){
  #   hm_axis.text.x = axis.text.x
  # } else {
  #   hm_axis.text.x = element_blank()
  # }
  # 
  # scaled_df = scaled_df[complete.cases(scaled_df),]
  # ZScore_heatmap = ggplot(scaled_df, aes(Sample_ID, Gene)) + geom_tile(aes(fill = ZScore), color = NA, position = 'identity', 
  #                                                                      width = tile_width_factor, height = tile_height_factor) + 
  #   facet_grid(.~groups, scales = "free_x", space = "free_x", switch = panel_label_switch) +
  #   scale_fill_gradientn(colours=zscore_colors, 
  #                        na.value = "white", limits = c(-extreme_sig, extreme_sig)) +      
  #   labs(x = axis_title_x_lab, y = axis_title_y_lab) +
  #   theme(
  #     panel.spacing = sample_panel_spacing, # adjust spacing between lines
  #     axis.text.x = hm_axis.text.x,
  #     axis.text.y = element_blank(),
  #     axis.title.x = axis_title_x,
  #     axis.title.y = axis_title_y,
  #     legend.position=left_legend_pos,
  #     # legend.key.size = unit(25,"pt"),
  #     legend.margin = zscore_legend_margin,
  ...
  #     plot.margin = zscore_plot_margin,
  #     strip.background = panel.strip.background,
  #     strip.text.x = panel.strip.text.x
  #   )
  # 
  # # ggsave(heatmap_save_path, ZScore_heatmap, device = "pdf", width = pdf_width, height = pdf_height)
  # 
  # categories = levels(factor(groups))
  # ave_group_df = data.frame(matrix(ncol = 3, nrow = 0))
  # names(ave_group_df) = c('Gene', 'Group', 'Ave')
  # 
  # 
  # 
  # for (category in categories) {
  #   category_df = data.frame(Gene = gene_names, 
  #                            Group = rep(category, length(gene_names)), 
  #                            Ave = apply(average_df[groups == category,  ],2, mean, na.rm = T))
  #   category_label = paste0("ave_zscore_", category)
  #   output_df[category_label, ] = c(category_label, category, category_df$Ave)
  #   ave_group_df = rbind(ave_group_df, category_df)
  # }
  # 
  # 
  # # display another map that shows the average gene expression for each group
  # 
  # ave_group_df$Gene = factor(ave_group_df$Gene, levels = rev(gene_names)) # backwards because the plot will start from the bottom up
  # ave_group_df$Group = factor(ave_group_df$Group %>% as.character, levels = categories)
  # ave_group_df$facet_group = "Average"
  # 
  # # get min and max for setting the scale bar
  # 
  # if(average_scaled_values){
  ...
  #   max_ave =  extreme_sig
  # } else {
  #   min_ave = min(ave_group_df$Ave, na.rm = T)
  #   max_ave = max(ave_group_df$Ave, na.rm = T)
  # }
  # axis.text.y.info = ifelse(axis.text.y.size %>% is_not_null,
  #                           paste0('element_text(size=', axis.text.y.size, ', hjust = 0)'),
  #                           'element_text(hjust = 0)')
  # # ave_group_df$Group2= ave_group_df$Group
  # 
  # if(length(axis_text_y_remove_strings) > 0){
  #   for(each_string in axis_text_y_remove_strings){
  #     ave_group_df$Gene = gsub( each_string, "", ave_group_df$Gene )
  #   }
  #   ave_group_df$Gene = factor(ave_group_df$Gene, levels = rev(unique(ave_group_df$Gene)))
  #   
  # }
  # 
  # if(axis_text_y_nchar %>% is_not_null){
  #   ave_group_df$Gene = ave_group_df$Gene %>% as.character
  #   for(gene_index in 1:length(ave_group_df$Gene)){
  #     my_gene = ave_group_df$Gene[gene_index]
  #      if(nchar(my_gene) > axis_text_y_nchar){
  #        my_gene = paste0(substring(my_gene,1,(axis_text_y_nchar-3)), "...")
  #        ave_group_df$Gene[gene_index] = my_gene
  #      }
  #   }
  #   ave_group_df$Gene = factor(ave_group_df$Gene, levels = rev(unique(ave_group_df$Gene)))
  # }
  # 
  # 
  # ave_heatmap = ggplot(ave_group_df, aes(Group, Gene)) + geom_tile(aes(fill = Ave), color = NA,
  #                                                                  width = tile_width_factor, height = tile_height_factor) + 
  #   scale_fill_gradientn(colours=average_colors, 
  #                        name = ave_legend_name,
  #                        na.value = "white", 
  #                        limits = c(min_ave, max_ave)) +     
  #   facet_grid(.~facet_group, scales = "free_x", space = "free_x", switch = panel_label_switch) +
  #   
  #   labs(x = "", y = "") +
  #   theme(
  #     panel.spacing = average_panel_spacing, # adjust spacing between lines
  #     # axis.text.x = element_blank(),
  #     axis.text.x = axis.text.x,  #TEMP
  #     axis.text.y = eval(parse(text= ifelse(display_all_names, axis.text.y.info, 'element_blank()'))),
  #     axis.title.y = element_text(size=20),
  #     legend.position=right_legend_pos,
  #     #legend.key.size = unit(25,"pt"),
  #     legend.margin = ave_legend_margin,
  #     legend.title = legend_title,
  #     legend.text = legend_text,
  #     legend.key.width = legend_key_width,
  #     legend.key.height = legend_key_height,
  ...
  #     axis.ticks = element_blank(),
  #     axis.line = element_blank(),
  #     plot.margin = ave_plot_margin,
  #     # getting rid of strip for the average 
  #     strip.background = element_rect(fill = 'white'), # higher number is lighter, border color change  (colour = 'red) wasn't working???
  #     strip.text.x = element_text(size=panel.strip.text.x$size, angle = 0, hjust = 0.5, vjust = 0, color = "white")
  #     #plot.margin = margin(0,0,0,1,"cm")#margin(t, r, b, l, "pt")
  #   ) 
  # 
  #   
  # # ggsave(heatmap_save_path, ave_heatmap, device = "pdf", width = pdf_width, height = pdf_height)
  # 
  # heat_maps = plot_grid(ZScore_heatmap, ave_heatmap, align = 'h', scale = plot_scale, rel_widths = graph_rel_widths)
  # 
  # if(base_title %>% is_not_null){
  #   heat_maps = heat_maps + 
  #     annotate("text", x=primary_title_options$x, 
  #              y=primary_title_options$y, 
  #              size=primary_title_options$size, 
  #              label=base_title) + 
  #     annotate("text", x=secondary_title$x, 
  #              y=secondary_title$y, 
  #              size=secondary_title$size, 
  #              label=secondary_title$label)
  # }
