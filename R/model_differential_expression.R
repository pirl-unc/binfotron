...
model_differential_expression = function(
  ...
  if(my_grouping %>% is.null){
    ...
    if(tolower(analysis_method) == "deseq"){
      ...
      #   Gene.Name = res$id,
      #   Gene.ID = get_gene_element(res$id, 2),
      #   Fold.Change = res$foldChange,
      #   FDR_pValue = res$padj,
      #   pValue = res$pval
      # )
      # 
      # output_stats = output_stats[ rev(order(output_stats$Fold.Change)), ]
      # 
      ...
      if ( deseq2_results_cooksCutoff %>% is_not_null){
      ...
      output_stats = data.frame(
        Gene.Name = sapply(rownames(res), function(x){strsplit(x, '|', fixed = TRUE)[[1]][1]}) %>% as.character, 
        Gene.ID = sapply(rownames(res), function(x){strsplit(x, '|', fixed = TRUE)[[1]][2]}) %>% as.character,
        Fold.Change = 2^res$log2FoldChange,
        FDR_pValue = p.adjust(res$pval, method = "BH"),
        pValue = res$pval
      )
    ...
    for(stats_col in stats_cols){
      ...
      for (gmt_cutoff in gmt_cutoffs){
        ...
        fold_by_col_1_names = sig_stats$Fold.Change
        names(fold_by_col_1_names) = sig_stats$Gene.Name %>% as.character()
        ...
        fold_by_col_2_names = sig_stats$Fold.Change
        names(fold_by_col_2_names) = sig_stats$Gene.ID %>% as.character()
    ...
    # rownames(output_stats) = output_stats$Gene.Name
    ...
    # heatmap_fold_change = output_stats[names(gene_dat), "Fold.Change"]
    # names(heatmap_fold_change) = names(gene_dat)
    # 
    # sig_genes = sig_stats$Gene.Name # get the list of genes that were significant in order of high to low fold change
    # #     heatmap_stats = data.frame(feature = pretty_gene_name(sig_stats$Gene.ID %>% as.character), pValue = sig_stats$FDR_pValue/100)
    # #     gene_dat = gene_dat[ , sig_genes %>% as.character] 
    # 
    # sig_stats$Gene.ID = sapply(sig_stats$Gene.Name %>% as.character, function(x){strsplit(x, '|', fixed = TRUE)[[1]][2]}) %>% as.character
    # sig_stats$Gene.Name = sapply(sig_stats$Gene.Name %>% as.character, function(x){strsplit(x, '|', fixed = TRUE)[[1]][1]}) %>% as.character
    # 
